...
async function getCartItemsByUserId() {
  try {
    ...
    const response = await fetch(
      "http://localhost:5000/api/cart/itemsByUserId",
      {
        headers: {
...
async function createOrderFromCart(req, res, next) {
  try {
    ...
    const response = await fetch(
      "http://localhost:5000/api/order/createFromCart",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
async function incrementCartItemQuantity(menuItemId) {
  ...
  try {
    const response = await fetch(
      `http://localhost:5000/api/cart/IncrementOne`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
async function decrementCartItemQuantity(menuItemId) {
  ...
  try {
    const response = await fetch(
      `http://localhost:5000/api/cart/DecrementOne`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
async function sendRecoveryCode(userEmail) {
  try {
    const response = await fetch(
      "http://localhost:5000/api/user/sendRecoveryCode",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
const verifyRecoveryCode = async (email, recoveryCode) => {
  try {
    const response = await fetch(
      "http://localhost:5000/api/user/verifyRecoveryCode",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
const updateUserData = async (name, email, password, phoneNumber) => {
  try {
    ...
    const response = await fetch(
      "http://localhost:5000/api/user/updateUserData",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
async function updateOrderStatus(id, newStatus) {
  ...
  try {
    ...
    const response = await fetch(
      "http://localhost:5000/api/order/updateStatus",
      requestOptions
    );
...
async function addMenuItem(
  ...
  const url = "http://localhost:5000/api/menu/add";
  const headers = {
    "Content-Type": "application/json",
    Authorization: `Bearer ${jwt}`,
  };
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.message || "Ошибка при добавлении позиции");
    }

    const result = await response.json();
    console.log("Позиция добавлена:", result);
    return result; // Возвращаем результат для дальнейшего использования
  } catch (error) {
    console.error("Ошибка:", error.message);
    throw error; // Пробрасываем ошибку для обработки в handleSave
  }
}
